...
func init() {
	m.Register(func(app core.App) error {
		...
		if err := json.Unmarshal([]byte(`{
			"viewQuery": "SELECT\n    t.deal as id,\n    t.deal,\n    (SUM(CASE WHEN t.type = 'financing' THEN ABS(t.amount) ELSE 0 END) * 100.0 /\n    NULLIF(SUM(CASE WHEN t.amount > 0 AND (t.type = 'revenue' OR t.type = '') THEN t.amount ELSE 0 END), 0)\n    ) as fundingAsPercentageOfRevenue\nFROM\n    transactions as t\nGROUP BY\n    deal;"
		}`), &collection); err != nil {
			return err
		}

		// remove field
		collection.Fields.RemoveById("_clone_vqmF")
		...
		// remove field
		collection.Fields.RemoveById("_clone_2IdI")
